...
from homework_04.models.db_async import async_session

fake = Faker()

module_models_user = homework.models.user
module_models_post = homework.models.post
...
async def test_main(users_data, posts_data):
    ...
    stmt_query_users = select(module_models_user.User).options(
        selectinload(module_models_user.User.posts)
    )
    stmt_query_posts = select(module_models_post.Post).options(
        joinedload(module_models_post.Post.user)
    )
