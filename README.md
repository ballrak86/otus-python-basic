# otus-python
1. [dz1_phone_book_func](https://github.com/ballrak86/otus-python-basic/tree/main/dz1_phone_book_func)

   Первое домашнее задание. Справочник с использованием функций.
2. [dz2_phone_book_class](https://github.com/ballrak86/otus-python-basic/tree/main/dz2_phone_book_class)

   Второе домашнее задание. Справочник с использованием классов.
3. [dz3_phone_book_pytest](https://github.com/ballrak86/otus-python-basic/tree/main/dz3_phone_book_pytest)

   Третье домашнее задание. Справочник с использованием тестов с модулем pytest.
4. [dz4_fastapi](https://github.com/ballrak86/otus-python-basic/tree/main/dz4_fastapi)

   Четвертое домашнее задание. Веб-приложение на FastAPI.
5. [dz5_docker](https://github.com/ballrak86/otus-python-basic/tree/main/dz5_docker)

   Пятое домашнее задание. Docker контейнер c веб-приложением.
6. [dz6_aio](https://github.com/ballrak86/otus-python-basic/tree/main/dz6_aio)

   Шестое домашнее задание. Асинхронная работа с сетью и БД.
7. [dz7_docker_compose](https://github.com/ballrak86/otus-python-basic/tree/main/dz7_docker_compose)

   Седьмое домашнее задание. Docker Compose взаимодействие между контейнерами.
8. [dz8_django_orm](https://github.com/ballrak86/otus-python-basic/tree/main/dz8_django_orm)

   Восьмое домашнее задание. Django ORM работа с БД через shell.
14. [dz14_course_project](https://github.com/ballrak86/otus-python-basic/tree/main/course_project)

   Курсовой проект. Веб-приложение ToDoList.
